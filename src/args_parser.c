...
ErrorCode free_args(ArgList *args) {
    if (args == NULL || args->argv == NULL) return ERR_NULL_POINTER;
...
/**
 * @brief Splits a command-line string into individual arguments, handling quotes.
 */
static ArgList split_args(const char *input) {
    ...
    while (*p) {
        ...
        if (quoted && *p == quoted) p++; // Skip closing quote
        token[pos] = '\0';
        ...
        if (result.argc >= MAX_TOKENS) break;
    }
    return result;
}
...
ErrorCode get_args(int *argc, char ***argv, const char *cmdline) {
    ...
    if (!args.argv) {
        return ERR_MEMORY_ALLOCATION; // expand_wildcards sets args to empty on failure
    }
